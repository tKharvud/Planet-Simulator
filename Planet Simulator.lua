...
function MapConstants:New()
	...
	mconst.polarFrontLatitude = 65		-- Default: 65
	mconst.tropicLatitudes = 23			-- Default: 23
	mconst.horseLatitudes = 31			-- Default: 31
	mconst.topLatitude = 70				-- Default: 70
	...
	mconst.minWaterTemp = 0.10 --Default: 0.10
	mconst.maxWaterTemp = 0.55 --Default: 0.50
...
function MapConstants:InitializeWorldAge()
	...
	elseif age == 3 then	--Old
		...
		self.hillsPercent = 0.65 		--Percent of dry land that is below the hill elevation deviance threshold.
...
function Cleanup()
	...
	for n=1,#desertTab do
		...
		if snowCount ~= 0 or forestCount > 0 then
			plot:SetTerrainType(terrainPlains,true,true)
			table.insert(plainsTab,i)
    ...
    -- fills in gaps in Forests + removes stray jungle tiles/bands touching desert
    for k = 1, #landTab do
        local i = landTab[k]
		local plot = Map.GetPlotByIndex(i)
		if not plot:IsMountain() then
						...
						if temperatureMap.data[i] > mc.jungleMinTemperature and plot:GetTerrainType() ~= terrainDesert then
							...
							for m=1,#tiless do
								local iii = tiless[m]
								local mPlot = Map.GetPlotByIndex(iii)
								if mPlot:GetTerrainType() == terrainDesert then
									desertCounts = desertCounts + 1
								end
							end
							if desertCounts > 2 then
								plot:SetFeatureType(nofeature,-1)
								if desertCounts > 7 and not(plot:IsRiver()) and not(plot:IsCoastalLand()) then
									plot:SetTerrainType(terrainDesert,true,true)
									table.insert(desertTab,i)
								end
							end
							-- Changes Jungle surrounded mostly by mountains to have grassland terrain, subtropical highland
							if plot:GetTerrainType() ~= terrainDesert and plot:GetFeatureType() ~= nofeature then
								local ntiles = GetCircle(i,1)
								local mountainCounts = 0
								local coastCounts = 0
								for nm=1,#ntiles do
									local ni = ntiles[nm]
									local nmplot = Map.GetPlotByIndex(ni)
									if nmplot:GetPlotType() == PlotTypes.PLOT_MOUNTAIN then
										mountainCounts = mountainCounts +1
									elseif nmplot:GetPlotType() == PlotTypes.PLOT_OCEAN then
										coastCounts = coastCounts +1
									end
								end
...
--~ 	end
	...
	for y=0,elevationMap.height - 1,1 do
		for x=0,elevationMap.width - 1,1 do
			...
			-- Sets artstyle depending on temperature
			if temperatureMap.data[i] < mc.snowTemperature and plot:IsMountain() then
				plot:SetContinentArtType(4)
			elseif temperatureMap.data[i] < mc.tundraTemperature and plot:IsMountain() then
				plot:SetContinentArtType(1)
